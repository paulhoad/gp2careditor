...
#define PROFILE(x, b) \
  x = pApp->GetProfileInt(strSection, #x, b) == 1 ? TRUE : FALSE;
...
void
Mak256SquareImage(CBitmap &jamBitmap, CBitmap &currentImg, int nWidth,
                  int nHeight)
{
  CDC memdcA, memdcB;
...
void
FlipVertically(CBitmap &jamBitmap, CBitmap &currentImg, int nWidth, int nHeight)
{
  CDC memdcA, memdcB;
  CBitmap *poldbmps, *poldbmpd;
  // Create a compatible memory DC
...
void
LoadJamBmp(CBitmap &output, int idx, char *filename)
{
  CBitmap temp;
  ...
  JamFile->LoadSingleBMPImage(temp, JamFile->getImageWidth(idx),
                              JamFile->getImageHeight(idx), idx);
  Mak256SquareImage(temp, output, JamFile->getImageWidth(idx),
                    JamFile->getImageHeight(idx));
}

void
CCarEditorView::ReReadJams()
{
...
CCarEditorView::UpdateJamFile()
{
  ...
  for (int j = 0; j < subimages; j++) {
    ...
    if (((CCarEditorApp *)AfxGetApp())->drawRCRJam) {
      ...
      JamFileStructure->LoadSingleBMPImage(
          jamBitmap, JamFileStructure->getImageWidth(j),
          JamFileStructure->getImageHeight(j), j);
    }
    else {
    ...
    TRACE("Jam Size = %d %d\n", JamFileStructure->getImageWidth(j),
          JamFileStructure->getImageHeight(j));
...
BOOL
CCarEditorView::PreCreateWindow(CREATESTRUCT &cs)
{
...
BOOL
CCarEditorView::OnPreparePrinting(CPrintInfo *pInfo)
{
  // default preparation
...
void
CCarEditorView::OnBeginPrinting(CDC * /*pDC*/, CPrintInfo * /*pInfo*/)
{
  // TODO: add extra initialization before printing
}

void
CCarEditorView::OnEndPrinting(CDC * /*pDC*/, CPrintInfo * /*pInfo*/)
{
...
#ifdef _DEBUG
void
CCarEditorView::AssertValid() const
{
  COpenGLView::AssertValid();
}

void
CCarEditorView::Dump(CDumpContext &dc) const
{
  COpenGLView::Dump(dc);
}
#endif
...
void
DrawText2D(HDC hdc, CScrollView *scview, GLdouble wx, GLdouble wy, GLdouble wz,
           int idx)
{
  GLdouble sx;
  GLdouble sy;
  ...
  ::TextOut(hdc, (int)(midx + winx), (int)(midy - winy), buffer,
            static_cast<int>(strlen(buffer)));
  glPopMatrix();
}

CCarEditorDoc *
CCarEditorView::GetDocument()  // non-debug version is inline
{
...
GLvoid
CalculateVectorNormal(GLdouble fVert1[], GLdouble fVert2[], GLdouble fVert3[],
                      GLdouble *fNormalX, GLdouble *fNormalY,
                      GLdouble *fNormalZ)
{
...
void
DrawTyre(CCarEditorDoc *pDoc, CCarEditorView *view, int id, BOOL front,
         BOOL left, BOOL showFilled)
{
  int wheelWidth = 70;
  int whellAcross = 90;
...
void
DrawCamera(Camera *camera, double wx, double wy, double wz, BOOL showAxis)
{
  glPushMatrix();
  GLUquadricObj *tyre = gluNewQuadric();
  glTranslated(wx, wy, wz);
...
void
DrawHelmet(CCarEditorDoc *pDoc, CCarEditorView *view, BOOL showFilled)
{
  int helmetWidth = 25;
  GLdouble wx;
  GLdouble wy;
...
t_TexturePos *
getTexture(int idx, BOOL selected, BOOL showTexture)
{
...
t_TexturePos *
getHelmetTexture(int idx, BOOL selected, BOOL showTexture)
{
  /*
  if (idx >= 390 && showTexture)
  {
...
double
getTX(t_TexturePos *text, int idx)
{
  double x = 0;
  x = (double)text->x[idx];
...
double
getTY(t_TexturePos *text, int idx)
{
  double x = 0;
  x = (double)text->y[idx];
...
double
getHTX(t_TexturePos *text, int idx)
{
  double x = 0;
  x = (double)text->x[idx];
  x += (hidx * 58);
...
double
getHTY(t_TexturePos *text, int idx)
{
  double x = 0;
  x = (double)text->y[idx];
  x += (hidy * 15);
...
void
CCarEditorView::OpenGLDrawScene(HDC hdc)
{
  CCarEditorDoc *pDoc = GetDocument();
  ...
  if (!Animation) {
    if (showNoseDetail) {
      // dc->SelectStockObject(ANSI_VAR_FONT);
      // if (pDoc->car && !pDoc->car->HiNose) dc->TextOut(10,10,"Viewing Low
      // Nose Car"); else dc->TextOut(10,10,"Viewing High Nose Car");
    }
  }
...
void
CCarEditorView::OnUpdateFillobjects(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showFilled);
}

void
CCarEditorView::OnShowPoints()
{
...
void
CCarEditorView::OnUpdateShowPoints(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showPoints);
}

void
CCarEditorView::OnShowtyres()
{
  showTyres = !showTyres;
  CCarEditorDoc *pDoc = GetDocument();
  if (pDoc) pDoc->UpdateAllViews(NULL);
...
void
CCarEditorView::OnUpdateShowtyres(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showTyres);
}

void
CCarEditorView::OnHelmet()
{
...
void
CCarEditorView::OnUpdateHelmet(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showHelmet);
}

void
CCarEditorView::OnDrawaxis()
{
  showAxis = !showAxis;
  CCarEditorDoc *pDoc = GetDocument();
  if (pDoc) pDoc->UpdateAllViews(NULL);
...
void
CCarEditorView::OnUpdateDrawaxis(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showAxis);
}

void
CCarEditorView::OnShowcameras()
{
...
void
CCarEditorView::OnUpdateShowcameras(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showCameras);
}

void
CCarEditorView::TextureMap(CBitmap &cbmp)
{
  BITMAP bmp;
  cbmp.GetBitmap(&bmp);
  ...
  if (!::GetDIBits(bitmapDc->GetSafeHdc(), HBITMAP(cbmp), 0, bmp.bmHeight,
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, bmp.bmWidth, bmp.bmHeight, 0, GL_RGB,
               GL_UNSIGNED_BYTE, lpvBits);
...
void
CCarEditorView::OnUpdateTexturemaps(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showTexture);
}

void
CCarEditorView::OnShowShadow()
{
...
void
CCarEditorView::OnUpdateShowShadow(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showShadowBox);
}

void
CCarEditorView::OnShowLines()
{
  showLines = !showLines;
  CCarEditorDoc *pDoc = GetDocument();
  if (pDoc) pDoc->UpdateAllViews(NULL);
...
void
CCarEditorView::OnUpdateShowLines(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showLines);
}

void
CCarEditorView::OnShowCulling()
{
...
void
CCarEditorView::OnUpdateShowCulling(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showCulling);
}

void
CCarEditorView::OnShowNormals()
{
...
void
CCarEditorView::OnUpdateShowNormals(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showNormals);
}

void
CCarEditorView::OnShowSingleColor()
{
...
void
CCarEditorView::OnUpdateShowSingleColor(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showSingleColor);
}

void
CCarEditorView::OnLighting()
{
...
void
CCarEditorView::OnUpdateLighting(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showLighting);
}

void
CCarEditorView::DrawHelmetModel()
{
  CCarEditorDoc *pDoc = GetDocument();
...
void
CCarEditorView::OnShowCar()
{
  CHECK_OBJECT(showCar);
}

void
CCarEditorView::OnUpdateShowCar(CCmdUI *pCmdUI)
{
  UPDATE_OBJECT(showCar);
}

void
CCarEditorView::OnShowPolygonNumbers()
{
  CHECK_OBJECT(showPolygonNumbers);
}

void
CCarEditorView::OnUpdateShowPolygonNumbers(CCmdUI *pCmdUI)
{
  UPDATE_OBJECT(showPolygonNumbers);
}

void
CCarEditorView::OnShowPointnumbers()
{
  CHECK_OBJECT(showPointNumbers);
}

void
CCarEditorView::OnUpdateShowPointnumbers(CCmdUI *pCmdUI)
{
  UPDATE_OBJECT(showPointNumbers);
}

void
CCarEditorView::OnShowVertexNumbers()
{
  CHECK_OBJECT(showVertexNumbers);
}

void
CCarEditorView::OnUpdateShowVertexNumbers(CCmdUI *pCmdUI)
{
  UPDATE_OBJECT(showVertexNumbers);
}

void
CCarEditorView::OnShowCameraNumbers()
{
  CHECK_OBJECT(showCameraNumbers);
}

void
CCarEditorView::OnUpdateShowCameraNumbers(CCmdUI *pCmdUI)
{
  UPDATE_OBJECT(showCameraNumbers);
}

void
CCarEditorView::OnJamSet()
{
...
CCarEditorView::getLookPoint()
{
  if (currentView == t_Air) {
    ...
    gluLookAt(eyeX, eyeY, (eyeZ), lookX, lookY, (lookZ), 0.0f, 0.0f, 1.0f);
...
void
CCarEditorView::OnCameraViewfromcamera1()
{
  CHECK_VIEW(t_Cam1)
}

void
CCarEditorView::OnCameraViewfromcamera2()
{
  CHECK_VIEW(t_Cam2)
}

void
CCarEditorView::OnCameraViewfromcamera3()
{
  CHECK_VIEW(t_Cam3)
}

void
CCarEditorView::OnCameraViewfromcamera4()
{
  CHECK_VIEW(t_Cam4)
}

void
CCarEditorView::OnCameraViewfromcamera5()
{
  CHECK_VIEW(t_Cam5)
}

void
CCarEditorView::OnCameraViewfromcamera6()
{
  CHECK_VIEW(t_Cam6)
}

void
CCarEditorView::OnCameraViewfromcamera7()
{
  CHECK_VIEW(t_Cam7)
}

void
CCarEditorView::OnCameraViewfromcamera8()
{
  CHECK_VIEW(t_Cam8)
}

void
CCarEditorView::OnCameraViewfromcamera9()
{
  CHECK_VIEW(t_Cam9)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera1(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam1)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera2(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam2)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera3(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam3)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera4(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam4)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera5(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam5)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera6(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam6)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera7(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam7)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera8(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam8)
}

void
CCarEditorView::OnUpdateCameraViewfromcamera9(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Cam9)
}

void
CCarEditorView::OnNormalView()
{
  CHECK_VIEW(t_Air)
}

void
CCarEditorView::OnUpdateNormalView(CCmdUI *pCmdUI)
{
  UPDATE_VIEW(t_Air)
}

void
CCarEditorView::OnCameraViewrightside()
{
  // top view
  // m_xRotate = -70;
  // m_yRotate = 0;
...
void
CCarEditorView::OnCameraViewleftside()
{
  m_xRotate = -70;
  m_yRotate = 0;
  m_zRotate = 90;
...
void
CCarEditorView::OnCameraViewfront()
{
  m_xRotate = -90;
  m_yRotate = 0;
  m_zRotate = 180;
...
void
CCarEditorView::OnCameraViewtop()
{
  // top view
  m_xRotate = 0;
  m_yRotate = 0;
...
void
CCarEditorView::OnCameraViewback()
{
  m_xRotate = -90;
  m_yRotate = 0;
  m_zRotate = 0;
...
void
CCarEditorView::OnCameraViewbottom()
{
  m_xRotate = 180;
  m_yRotate = 0;
  m_zRotate = 90;
...
void
CCarEditorView::OnCameraViewfrontleft()
{
  m_xRotate = -70;
  m_yRotate = 0;
  m_zRotate = 135;
...
void
CCarEditorView::OnCameraViewfrontright()
{
  m_xRotate = -70;
  m_yRotate = 0;
  m_zRotate = 225;
...
void
CCarEditorView::OnCameraViewbackleft()
{
  m_xRotate = -70;
  m_yRotate = 0;
  m_zRotate = 45;
...
void
CCarEditorView::OnCameraViewbackright()
{
  m_xRotate = -70;
  m_yRotate = 0;
  m_zRotate = 315;
...
void
CCarEditorView::OnStartAnimation()
{
...
void
CCarEditorView::OnTimer(UINT_PTR nIDEvent)
{
  if (Animation == FALSE) {
  }
  else {
...
void
CCarEditorView::OnLButtonDown(UINT nFlags, CPoint point)
{
  KillTimer(0);
  Animation = FALSE;
...
void
CCarEditorView::OnUpdateStartAnimation(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(Animation);
}

void
CCarEditorView::OnZoomOut()
{
  dist = dist * 1.2;
  InvalidateRect(NULL, FALSE);
}
...
int
CCarEditorView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  if (COpenGLView::OnCreate(lpCreateStruct) == -1) return -1;
...
void
CCarEditorView::OnUpdateShowNoseCone(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(showNoseDetail);
}

void
CCarEditorView::OnJamExportrcrjam()
{
  // TODO: Add your command handler code here
...
void
CCarEditorView::OnUpdateShowObject(CCmdUI *pCmdUI)
{
...
void
CCarEditorView::DrawObjectModel()
{
  CCarEditorDoc *pDoc = GetDocument();
...
void
CCarEditorView::OnJamDrawrcrjam()
{
  ((CCarEditorApp *)AfxGetApp())->drawRCRJam =
      !((CCarEditorApp *)AfxGetApp())->drawRCRJam;
  // ReReadJams();
  UpdateJamFile();
}
...
void
CCarEditorView::OnUpdateJamDrawrcrjam(CCmdUI *pCmdUI)
{
  pCmdUI->SetCheck(((CCarEditorApp *)AfxGetApp())->drawRCRJam);
}

void
CCarEditorView::OnJamProducercrimages()
{
  m_xRotate = -90;
  m_yRotate = 0;
  m_zRotate = 0;
