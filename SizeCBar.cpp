...
void
CSizingControlBar::OnUpdateCmdUI(class CFrameWnd* pTarget,
...
BOOL
CSizingControlBar::Create(CWnd* pParentWnd, CSize sizeDefault, UINT nID,
                          DWORD dwStyle)
{
  ...
  if (!CWnd::Create(wndclass, NULL, dwStyle, CRect(0, 0, 0, 0), pParentWnd,
                    nID))
    return FALSE;
...
CSize
CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
  CRect rc;
...
CSize
CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void
CSizingControlBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos)
{
...
BOOL
CSizingControlBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
//
void
CSizingControlBar::OnLButtonUp(UINT nFlags, CPoint point)
{
  if (!m_bTracking)
...
void
CSizingControlBar::OnMouseMove(UINT nFlags, CPoint point)
{
...
void
CSizingControlBar::OnNcCalcSize(BOOL bCalcValidRects,
...
void
CSizingControlBar::OnNcLButtonDown(UINT nHitTest, CPoint point)
{
...
LRESULT
CSizingControlBar::OnNcHitTest(CPoint point)
{
  if (IsFloating()) return CControlBar::OnNcHitTest(point);
...
void
CSizingControlBar::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void
CSizingControlBar::OnLButtonDblClk(UINT nFlags, CPoint point)
{
...
void
CSizingControlBar::StartTracking()
{
  SetCapture();
...
void
CSizingControlBar::OnCaptureChanged(CWnd* pWnd)
{
  if (m_bTracking && pWnd != this) StopTracking(FALSE);  // cancel tracking

  CControlBar::OnCaptureChanged(pWnd);
}

void
CSizingControlBar::StopTracking(BOOL bAccept)
{
  OnInvertTracker(m_rectTracker);
...
void
CSizingControlBar::OnInvertTracker(const CRect& rect)
{
  ASSERT_VALID(this);
  ...
  CDC* pDC =
      m_pDockSite->GetDCEx(NULL, DCX_WINDOW | DCX_CACHE | DCX_LOCKWINDOWUPDATE);
...
BOOL
CSizingControlBar::IsHorz() const
{
...
CPoint&
CSizingControlBar::ClientToWnd(CPoint& point)
{
