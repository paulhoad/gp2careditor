...
void
GPOObject::LoadTree(CCarTree *tree)
{
  char buffer[256];
  carNode = tree->insertInfoNode(tree->m_CarTree.GetRootItem(), "GPO Object",
                                 TO_ID(IDB_HELMET));
  ...
  for(i = 0; i< textureSize;i++)
  {
          ...
          wsprintf(buffer,"[%d] %d %d (0x%x)
  %s",i,textureData[i].numl,textureData[i].numh,textureData[i].cmd,argsStr);
          HTREEITEM textItem =
  tree->insertInfoNode(textureP,buffer,TO_ID(IDB_POLYGONS)); HTREEITEM
  textureItem = textureData[i].LoadTree(tree,textItem); textureData[i].node =
  textItem; textureData[i].idx = i;
...
void
GPOObject::updateData(BOOL updateTexture)
{
  int start = 78;
