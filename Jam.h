...
class JAMEntry : public CObject
{
   ...
   unsigned char *AdjustImage(int size,unsigned char *imgPtr,BOOL rcr);
   ...
   void LoadTree(CCarTree *tree,HTREEITEM root)
	{
		char buffer[1024];

		wsprintf(buffer,"JamId:%d  (%d %d %d %d)",info->jam_id,info->x,info->y,info->width,info->height);
		...
		for(int i=0;i<info->palette_size_div4;i++)
		{
		  ...
		  wsprintf(buffer,"[%d] Val:%d (%d %d %d)",i,(int)*ptr,palette[(int)*ptr].r,palette[(int)*ptr].g,palette[(int)*ptr].b);
		  ptr++;
...
class JAM : public CObject
{
   public:
	...
	BOOL DrawAllJams(CDC *pDC,double currentScale, BOOL drawRcr,BOOL drawOutlines=TRUE);
	BOOL DrawSingleJams(CDC *pDC,double currentScale,CBitmap &currentJam,BOOL rcr);
	...
	HTREEITEM LoadTree(CCarTree *tree,HTREEITEM root,BOOL loaded)
	{
		char buffer[128];
