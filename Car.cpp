	 ...
	 for(int i=0;i<numPoints;i++)
	 {
		...
		switch(i)
		{
		   case 63 :  pointData[i].setDesc("Helmet Position");
					  break;
		   case 42 :  pointData[i].setDesc("Tire LF Position");
					  break;
		   case 43 :  pointData[i].setDesc("Tire LR Position");
					  break;
		   case 44 :  pointData[i].setDesc("Tire RF Position");
					  break;
		   case 45 :  pointData[i].setDesc("Tire RR Position");
					  break;
		}
	  }
	 }
...
void Car::LoadTree(CCarTree *tree,CDocument *pDoc)
{
	...
	for(int i=0;i<9;i++)
	{
	   ...
	   wsprintf(buffer,"c[%d] (%d,%d,%d) (D=%d P=%d R=%d) (%d %d)",i+1,cameraData[i].pos.X,
