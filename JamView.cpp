...
void CJamView::OnDraw(CDC* pDC)
{
	...
	if (myjam)
	{
      ...
      if (myjam->rcr) myjam->DrawSingleJams(pDC,currentScale,pDoc->currentJamBitmap,myjam->rcr);	  
	  else myjam->DrawAllJams(pDC,currentScale,rcrJam,showNumbers);	  
	}
}
...
void CJamView::OnShowJamnumbers() 
{
	showNumbers = !showNumbers;
	InvalidateRect(NULL,FALSE);
}

void CJamView::OnUpdateShowJamnumbers(CCmdUI* pCmdUI) 
{
...
void CJamView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (myjam)
	{
	 ...
	 char msg[256];
	 wsprintf(msg,"[%d] = RGB(%d,%d,%d)",idx,GetRValue(clr),GetGValue(clr),GetBValue(clr));
