...
void CCarTree::OnSize(UINT nType, int cx, int cy) 
{
...
void CCarTree::Reposition(CRect rc)
{
	int border = 4;

    int bottom = (IsHorz() || IsFloating())
        ? rc.Height() - (2*border) : rc.Height() - (2*border) - 4;

    m_CarTree.SetWindowPos(NULL,rc.left + border, rc.top + border,
            rc.Width() - (2*border), bottom , SWP_NOACTIVATE | SWP_NOZORDER);            
}

int CCarTree::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CCarTree::InsertCar(CDocument *doc,Car *car)
{
...
void CCarTree::InsertHelmet(CDocument *doc,Helmet *helmet)
{
	helmet->pDoc = doc;
...
void CCarTree::InsertGPOObject(CDocument *doc,GPOObject *object)
{
	object->pDoc = doc;
...
HTREEITEM CCarTree::insertInfoNode(HTREEITEM parent, LPSTR title,int bitmap)
{
	if (m_CarTree)
	{
		TV_INSERTSTRUCT tvstruct;
...
void CCarTree::OnDblclkCartree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (m_CarTree)
	{
...
void CCarTree::OnclkCartree(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (m_CarTree)
	{
	 HTREEITEM TreeNode = m_CarTree.GetSelectedItem();
