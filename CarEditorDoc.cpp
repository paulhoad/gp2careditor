...
void
CCarEditorDoc::Serialize(CArchive &ar)
{
  if (ar.IsStoring()) {
...
#ifdef _DEBUG
void
CCarEditorDoc::AssertValid() const
{
  CDocument::AssertValid();
}

void
CCarEditorDoc::Dump(CDumpContext &dc) const
{
  CDocument::Dump(dc);
}
#endif  //_DEBUG
...
BOOL
CCarEditorDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
...
void
CCarEditorDoc::DeleteContents()
{
  if (car != NULL && car->carNode) {
    ((CCarEditorApp *)AfxGetApp())
        ->m_MainTree->m_CarTree.DeleteItem(car->carNode);
  }
  if (helmet != NULL && helmet->carNode) {
    ((CCarEditorApp *)AfxGetApp())
        ->m_MainTree->m_CarTree.DeleteItem(helmet->carNode);
  }
}

void
CCarEditorDoc::OnUpdateHighNose(CCmdUI *pCmdUI)
{
  if (car != NULL) pCmdUI->SetCheck(car->HiNose);
}

void
CCarEditorDoc::OnCopyCar1ToCar2()
{
...
void
CCarEditorDoc::OnUpdateCarone(CCmdUI *pCmdUI)
{
  if (car != NULL) pCmdUI->SetCheck(!car->HiNose);
}

void
CCarEditorDoc::OnCartwo()
{
  if (car) {
    car->HiNose = TRUE;
    car->updateData(FALSE);
...
void
CCarEditorDoc::OnUpdateCartwo(CCmdUI *pCmdUI)
{
  if (car != NULL) pCmdUI->SetCheck(car->HiNose);
}

void
CCarEditorDoc::OnFileSave()
{
  OnFileSaveAs();
}

void
CCarEditorDoc::OnJamView()
{
  ((CCarEditorApp *)AfxGetApp())->OnJamView(this);
}

char lpszFileHelmetFilter[] =
    "F1GP2 Helmet Files (*.hlt)|*.hlt|All Files (*.*)|*.*|";

void
CCarEditorDoc::OnFileImporthelmet()
{
  char fileName[256];

  strcpy(fileName, "unnamed.hlt");
  CFileDialog *fdlg = new CFileDialog(TRUE, "hlt", (LPCSTR)&fileName,
                                      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                                      lpszFileHelmetFilter);
...
void
CCarEditorDoc::OnSaveHelmet()
{
  if (helmet) {
    char fileName[256];

    strcpy(fileName, "unnamed.hlt");
    CFileDialog *fdlg = new CFileDialog(FALSE, "hlt", (LPCSTR)&fileName,
                                        OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                                        lpszFileHelmetFilter);
...
char lpszFileGPOFilter[] =
    "F1GP2 Object Files (*.gpo)|*.gpo|All Files (*.*)|*.*|";

void
CCarEditorDoc::OnFileImportgpoobject()
{
  char fileName[256];

  strcpy(fileName, "*.gpo");
  CFileDialog *fdlg = new CFileDialog(TRUE, "gpo", (LPCSTR)&fileName,
                                      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                                      lpszFileGPOFilter);
  ...
  if (result == IDOK) {
    ...
    if (gpoobject)
      ((CCarEditorApp *)AfxGetApp())
          ->m_MainTree->InsertGPOObject(this, gpoobject);
  }
}
...
GPOObject *
CCarEditorDoc::ImportGPOObject(CString filename)
{
